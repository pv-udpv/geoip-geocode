...
class ProviderDefaults:
    ...
    DEFAULT_DATABASE_PATHS = [
        "./data/databases/GeoLite2-City.mmdb",
        "./GeoLite2-City.mmdb",
...
def create_provider_config_safe(
    ...
    return ProviderConfig(
        name=name, enabled=enabled, database_path=database_path, priority=priority
    )
...
def example_improved_basic():
    ...
    try:
        ...
        provider_config = ProviderConfig(
            ...
            enabled=True,
            database_path=str(
                find_database_file() or ProviderDefaults.DEFAULT_DATABASE_PATHS[0]
            ),
...
def example_improved_safe():
    ...
    try:
        ...
        provider_config = create_provider_config_safe(validate_path=False)
        config.add_provider_config(provider_config)
...
def example_improved_builder():
    ...
    try:
        ...
        config.add_provider_config(provider_config)
        print(
            "✓ Improvements: Builder pattern, fluent interface, flexible configuration"
        )
...
def example_environment_aware():
    ...
    try:
        ...
        config.add_provider_config(provider_config)
        print(
            "✓ Improvements: Environment variable support, configurable without code changes"
        )
