...
def example_with_caching():
    ...
    try:
        ...
        for ip in test_ips:
            ...
            if result:
                print(
                    f"  {ip}: {result.city}, {result.country_name} (ASN: {result.asn})"
                )
...
def example_cache_management():
    ...
    cache_config = CacheConfig(
        enabled=True, max_size=100, ttl=3600  # Small cache for demonstration
    )
...
def example_performance_comparison():
    ...
    try:
        ...
        provider_no_cache = MultiDatabaseGeoIP2Provider(
            provider_config, CacheConfig(enabled=False)
        )
        ...
        provider_with_cache = MultiDatabaseGeoIP2Provider(
            provider_config, CacheConfig(enabled=True, max_size=1000, ttl=3600)
        )
...
def example_backward_compatibility():
    ...
    config = ProviderConfig(
        name="geoip2-compat", database_path="./GeoLite2-City.mmdb"  # Old parameter name
    )
    ...
    try:
        ...
        for ip in ip_list:
            ...
            if result:
                ...
                print(
                    f"  ✓ {ip}: {result.city}, {result.country_code} (ASN: {result.asn})"
                )
            else:
